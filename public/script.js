...
function updateRoomPill(code) {
  ...
  // show pill once you're in a room
...
if (openLobbyBtnEl) {
  openLobbyBtnEl.addEventListener("click", () => {
    showLobbyOverlay();
  });
}
...
function initOnline() {
  ...
  ONLINE.socket.on("disconnect", () => {
    ...
    showLobbyOverlay();
    updateRoomPill(null);
    if (closeLobbyBtnEl) closeLobbyBtnEl.style.display = "none";
  });
  ...
  ONLINE.socket.on("roomJoined", (payload) => {
    ...
    setLobbyStatus(ONLINE.isHost ? "Room created. Share the code." : "Joined room. Waiting for host to start.");
    showRoomInfo(code, list);

    if (closeLobbyBtnEl) closeLobbyBtnEl.style.display = "inline-flex";
    updateRoomPill(code);

    // Host uses the existing setup overlay.
    if (ONLINE.isHost) {
      enableSetupForHost();
  ...
  ONLINE.socket.on("hostAction", ({ type, payload }) => {
    ...
    // Validate turn ownership if seat is known
    if (requesterSeat !== null && requesterSeat !== activeIdx) {
      // Ignore out-of-turn requests
      return;
    }
