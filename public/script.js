...
if (startGameBtn && setupScreen) {
  startGameBtn.addEventListener("click", () => {
    ...
    // If the online lobby overlay is still open, close it so the board is playable.
    hideLobbyOverlay();
...
// =====================
// ONLINE MULTIPLAYER (ROOM CODE + SYNC)
// =====================
// Design choice (simple + reliable):
// - The first player to create the room is the HOST.
// - The HOST is authoritative: the host runs the full existing game logic.
// - Non-host players can request actions (Roll / End Turn / Bankruptcy).
// - The HOST applies the action locally and broadcasts a full state snapshot.
//
// This keeps your existing 1-computer game logic intact while making it playable online.
...
const lobbyStartGameBtnEl = document.getElementById("lobbyStartGameBtn");

// Color picker DOM
const colorOverlayEl = document.getElementById("colorOverlay");
const colorGridEl = document.getElementById("colorGrid");
const colorConfirmBtnEl = document.getElementById("colorConfirmBtn");
...
function myDisplayName() {
  const raw = (lobbyNameEl && lobbyNameEl.value || "").trim();
  return raw ? raw.slice(0, 15) : "Player";
}
...
function renderRoomPlayers(list) {
  ...
  (list || []).forEach(p => {
    ...
    const seatTag = (typeof p.seat === "number") ? ` [P${p.seat + 1}]` : "";
    li.textContent = `${p.name || "Player"}${hostTag}${seatTag}`;
...
function updateLobbyStartButton() {
  if (!lobbyStartGameBtnEl) return;
  const count = Array.isArray(ONLINE.roomPlayers) ? ONLINE.roomPlayers.length : 0;
  const canStart = ONLINE.isHost && count >= 2;
  lobbyStartGameBtnEl.disabled = !canStart;
  lobbyStartGameBtnEl.style.opacity = canStart ? "1" : "0.55";
}
...
// Close lobby overlay (X button or click outside the card)
if (closeLobbyBtnEl) {
  closeLobbyBtnEl.addEventListener("click", () => {
    hideLobbyOverlay();
  });
}
if (lobbyOverlayEl) {
...
if (colorConfirmBtnEl) {
  colorConfirmBtnEl.addEventListener("click", () => {
    if (!ONLINE.enabled) return;
    if (!ONLINE.gameStarted) return;
    if (ONLINE.seat == null) return;
    const hex = ONLINE.pendingColor;
    if (!hex) return;

    // Host can set directly; guests request host
    if (ONLINE.isHost) {
      // ensure uniqueness
      const taken = new Set(players.filter(p => p.active).map(p => p.color));
      taken.delete(players[ONLINE.seat].color);
      if (!taken.has(hex)) {
        players[ONLINE.seat].color = hex;
        renderBoard();
        updateMoneyUI();
        broadcastSnapshot();
      }
      ONLINE.colorChosen = true;
      hideColorPicker();
    } else {
      requestHostAction("SET_COLOR", { hex });
      ONLINE.colorChosen = true;
      hideColorPicker();
    }
  });
...
function disableSetupForNonHost() {
  if (!startGameBtn) return;
  startGameBtn.disabled = true;
  startGameBtn.style.opacity = "0.5";
}

function enableSetupForHost() {
  if (!startGameBtn) return;
  startGameBtn.disabled = false;
  startGameBtn.style.opacity = "1";
}

function lockControlsIfNotYourTurn() {
  // NOTE: host stays fully interactive.
  // Non-host: only allow interaction if it's your turn AND the host is running.
  if (!ONLINE.enabled) return;
  if (ONLINE.isHost) return;

  const activeIdx = turnOrder && turnOrder.length ? turnOrder[currentTurnPointer] : null;
  const myIdx = ONLINE.seat;

  const myTurn = (activeIdx === myIdx);

  // Roll / End Turn / Bankruptcy are the only buttons we forward.
  if (rollBtn) rollBtn.disabled = !myTurn;
  if (endTurnBtn) endTurnBtn.disabled = !myTurn;
  if (bankruptBtn) bankruptBtn.disabled = !myTurn;

  // Trade is complex (multiple confirmations) — keep host-driven for now.
  if (tradeBtn) tradeBtn.disabled = true;
}

function buildSnapshot() {
  // Keep snapshot purely JSON-safe.
  return {
    v: 1,
...
function applySnapshot(snap) {
  ...
  ONLINE.suppressLocalSync = true;

  try {
    // Assign globals (these are declared with let/const above)
    ...
    auctionTileIndex = (typeof snap.auctionTileIndex === "number") ? snap.auctionTileIndex : auctionTileIndex;
    auctionPlayers = Array.isArray(snap.auctionPlayers) ? snap.auctionPlayers : auctionPlayers;
    auctionCurrentPlayerPtr = (typeof snap.auctionCurrentPlayerPtr === "number") ? snap.auctionCurrentPlayerPtr : auctionCurrentPlayerPtr;
    auctionHighestBid = (typeof snap.auctionHighestBid === "number") ? snap.auctionHighestBid : auctionHighestBid;
    auctionHighestBidderIdx = (typeof snap.auctionHighestBidderIdx === "number") ? snap.auctionHighestBidderIdx : auctionHighestBidderIdx;
    auctionPassed = Array.isArray(snap.auctionPassed) ? snap.auctionPassed : auctionPassed;

    chanceDeck = Array.isArray(snap.chanceDeck) ? snap.chanceDeck : chanceDeck;
    communityDeck = Array.isArray(snap.communityDeck) ? snap.communityDeck : communityDeck;

    board = Array.isArray(snap.board) ? snap.board : board;

    // Event log
    if (Array.isArray(snap.eventLogEntries)) {
      eventLogEntries.length = 0;
      snap.eventLogEntries.forEach(x => eventLogEntries.push(x));
    }
    ...
    if (currentTurnDisplay && activePlayer) {
      currentTurnDisplay.textContent = `Current Turn: ${activePlayer.name}`;
    }

    renderEventLog();
    renderBoard();
    renderPropertyInfo(selectedTileIndex);
    updateHoldingsPanel();
    setupTradePanel();

    lockControlsIfNotYourTurn();
    maybeShowColorPicker();
  } finally {
    ONLINE.suppressLocalSync = false;
  }
}


function maybeShowColorPicker() {
  if (!ONLINE.enabled) return;
  if (!ONLINE.gameStarted) return;
  if (ONLINE.colorChosen) return;
  if (!colorOverlayEl || !colorGridEl || !colorConfirmBtnEl) return;
  if (ONLINE.seat == null) return;

  const me = players && players[ONLINE.seat];
  if (!me || !me.active) return;

  // Build available colors (use existing COLOR_MAP)
  const active = players.filter(p => p.active);
  const taken = new Set(active.map(p => p.color).filter(Boolean));

  const entries = Object.entries(COLOR_MAP || {});
  if (!entries.length) return;

  colorGridEl.innerHTML = "";
  let selectedHex = me.color || null;

  const choose = (hex, el) => {
    if (!hex) return;
    selectedHex = hex;
    ONLINE.pendingColor = hex;
    [...colorGridEl.querySelectorAll(".color-swatch")].forEach(x => x.classList.remove("selected"));
    el.classList.add("selected");
    colorConfirmBtnEl.disabled = false;
  };
  ...
  entries.forEach(([name, hex]) => {
    ...
    const isTakenByOther = taken.has(hex) && hex !== me.color;
    if (isTakenByOther) {
      sw.classList.add("disabled");
      sw.disabled = true;
    }

    if (hex === selectedHex) sw.classList.add("selected");

    sw.addEventListener("click", () => choose(hex, sw));
    colorGridEl.appendChild(sw);
  });

  colorConfirmBtnEl.disabled = !selectedHex;
  ONLINE.pendingColor = selectedHex;

  colorOverlayEl.style.display = "flex";
}

function hideColorPicker() {
  if (colorOverlayEl) colorOverlayEl.style.display = "none";
}
...
function requestHostAction(type, payload) {
  if (!ONLINE.enabled) return;
  if (!ONLINE.socket) return;
  if (ONLINE.isHost) return; // host does not request
...
function initOnline() {
  ...
  if (typeof io === "undefined") {
    setLobbyStatus("Online server not detected. Start the server and refresh.");
    return;
  }
  ...
  ONLINE.socket.on("disconnect", () => {
    ...
    setLobbyStatus("Disconnected. Refresh after restarting server.");
    showLobbyOverlay();
  });
  ...
  ONLINE.socket.on("roomUpdate", ({ code, players: list }) => {
    ...
    if (ONLINE.roomCode && roomCodeTextEl) roomCodeTextEl.textContent = ONLINE.roomCode;
    renderRoomPlayers(list);
  ...
  ONLINE.socket.on("roomJoined", (payload) => {
    ...
    ONLINE.roomPlayers = Array.isArray(list) ? list : [];
    ONLINE.gameStarted = false;
    ONLINE.colorChosen = false;
    ONLINE.pendingColor = null;

    setLobbyStatus(ONLINE.isHost ? "Room created. Share the code, then press Start Online Game." : "Joined room. Waiting for host to start.");
    showRoomInfo(code, list);

    // For online play, we keep the old Setup overlay hidden.
  ...
  ONLINE.socket.on("hostState", (snap) => {
    // Everyone can receive snapshots (host echo is harmless)
    ONLINE.gameStarted = true;
    hideLobbyOverlay();
    applySnapshot(snap);
  });

  ONLINE.socket.on("hostAction", ({ type, payload, fromSeat }) => {
    // Host receives requests from other clients
    if (!ONLINE.isHost) return;

    const activeIdx = turnOrder && turnOrder.length ? turnOrder[currentTurnPointer] : 0;
    const requesterSeat = (typeof fromSeat === "number") ? fromSeat : null;

    // Colors can be chosen anytime (not turn-based)
    if (type === "SET_COLOR") {
      const hex = payload && typeof payload.hex === "string" ? payload.hex : null;
      if (hex && players && players[requesterSeat]) {
        // ensure uniqueness among active players
        const taken = new Set(players.filter(p => p.active).map(p => p.color));
        // allow keeping your current color
        taken.delete(players[requesterSeat].color);
        if (!taken.has(hex)) {
          players[requesterSeat].color = hex;
          renderBoard();
          updateMoneyUI();
          broadcastSnapshot();
        }
    ...
    // Validate turn ownership for gameplay actions
    if (requesterSeat !== null && requesterSeat !== activeIdx) {
      return; // ignore out-of-turn requests
    }
  ...
  if (joinCodeInputEl) {
    joinCodeInputEl.addEventListener("input", () => {
      joinCodeInputEl.value = sanitizeCode(joinCodeInputEl.value);
    });
  }
  ...
  if (createRoomBtnEl) {
    createRoomBtnEl.addEventListener("click", () => {
      ...
      ONLINE.socket.emit("createRoom", { name: myDisplayName() });
  ...
  if (joinRoomBtnEl) {
    joinRoomBtnEl.addEventListener("click", () => {
      ...
      const code = sanitizeCode(joinCodeInputEl ? joinCodeInputEl.value : "");
      if (!code) {
        setLobbyStatus("Enter a room code.");
        return;
      }
      ONLINE.socket.emit("joinRoom", { code, name: myDisplayName() });
  ...
  if (copyRoomCodeBtnEl) {
    copyRoomCodeBtnEl.addEventListener("click", async () => {
      try {
        await navigator.clipboard.writeText(String(ONLINE.roomCode || ""));
        setLobbyStatus("Room code copied.");
      } catch {
  ...
  if (lobbyStartGameBtnEl) {
    lobbyStartGameBtnEl.addEventListener("click", () => {
      if (!ONLINE.enabled || !ONLINE.socket || !ONLINE.connected) {
        setLobbyStatus("Not connected to server.");
        return;
      }
      if (!ONLINE.isHost) return;
      ...
      if (list.length < 2) {
        setLobbyStatus("Need at least 2 players in the room.");
        return;
      }

      // Fill the hidden Setup overlay inputs so we can reuse your existing Start Game logic.
      const numPlayersSelect = document.getElementById("numPlayersSelect");
      if (numPlayersSelect) numPlayersSelect.value = String(Math.min(4, Math.max(2, list.length)));

      // Push room names into Player 1..N inputs
      const inputs = [document.getElementById("player1NameInput"), document.getElementById("player2NameInput"), document.getElementById("player3NameInput"), document.getElementById("player4NameInput")];
      inputs.forEach((inp, idx) => {
        if (!inp) return;
        inp.value = (list[idx] && list[idx].name) ? String(list[idx].name).slice(0, 15) : "";
      });

      // Start the game locally (host authoritative)
      if (startGameBtn) {
        startGameBtn.click();
      }

      ONLINE.gameStarted = true;
      ONLINE.colorChosen = false; // everyone will get a color picker after start (optional)

      // Immediately broadcast a fresh snapshot so guests auto-sync.
      setTimeout(broadcastSnapshot, 0);
    });
  }


  // Prevent non-host from triggering local game setup.
  if (startGameBtn) {
    startGameBtn.addEventListener(
      "click",
      (e) => {
        if (!ONLINE.enabled) return;
        if (ONLINE.isHost) return; // host allowed
  ...
  // Forward non-host controls to host (capture-phase, so we don\'t touch your existing listeners).
  const forwardIfNonHost = (el, actionType) => {
    if (!el) return;
    el.addEventListener(
      "click",
      (e) => {
  ...
  // If host, broadcast state after any click that might mutate game.
  // (This is intentionally broad; it keeps syncing simple.)
  if (document && document.addEventListener) {
    document.addEventListener("click", () => {
      if (!ONLINE.enabled) return;
      if (!ONLINE.isHost) return;
      // Slight delay so the click handler finishes updates before snapshotting.
      setTimeout(broadcastSnapshot, 0);
    });
  }

  // Start with setup disabled until we know if we\'re host.
  disableSetupForNonHost();
  showLobbyOverlay();
}
